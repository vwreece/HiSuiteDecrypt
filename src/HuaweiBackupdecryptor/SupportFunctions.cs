...
namespace HuaweiBackupdecryptor
{
    ...
    public static class kobackupdec
    {
        ...
        static HybridDictionary parse_info_xml(string filepath, ref Decryptor decryptor, HybridDictionary decrypt_material_dict)
        {
            ...
            {
                Console.WriteLine($"First tag should be 'info.xml', not '{info_dom.FirstChild.Name}'");
                decryptor = null;
                return null;
            }
            ...
            foreach (XmlNode entry in elemList)
            {
                ...
                switch (title)
                {
                    ...
                    case "BackupFileModuleInfo_SystemData":
                        ...
                        break;
                    default:
                        Console.WriteLine($"Unknown entry in 'info.xml': {title}");
                        break;
                }
            }
        ...
        internal static void decrypt(string user_password, string backup_path_in, string dest_path_out)
        {
            ...
            if (Directory.Exists(dest_path_out))
            {
                ...
                if (files.Count > 0)
                {
                    Console.WriteLine($"output folder contains {files.Count} files, cannot overwrite them!");
                    return;
                }
            }
            ...
            if (tar_files.Count > 0)
            {
                ...
                foreach (string entry in tar_files)
                {
                    ...
                    else Console.WriteLine($"entry '{skey}' has no decrypt material!");
            ...
            if (enc_files.Count > 0)
            {
                ...
                for (int i = 1; i <= enc_files.Count; i++)
                {
                    ...
                    if (decrypt_material_dict.Contains(skey))
                    {
                        ...
                        Console.Write($"\r{aString}{asterisk[i % asterisk.Length]}");
    ...
    internal class Decryptor
    {
        ...
        internal void crypto_init()
        {
            ...
            Console.WriteLine($"SHA256(BKEY)[{this._bkey_sha256.Length}] = {DecryptMaterial.hexlify(this._bkey_sha256)}");

            byte[] salt = this._checkMsgBytes.Skip(32).ToArray();
            Console.WriteLine($"SALT[{salt.Length}] = {DecryptMaterial.hexlify(salt)}");

            byte[] res = PBKDF2_SHA256_GetBytes(Encoding.UTF8.GetBytes(this._bkey), salt, Decryptor.dklen, Decryptor.count);
            Console.WriteLine($"KEY check expected = {DecryptMaterial.hexlify(this._checkMsgBytes.Take(32).ToArray())}");
            Console.WriteLine($"RESULT = {DecryptMaterial.hexlify(res)}");
